...
=======================
This tutorial demonstrates how to use the **WaveSpace** toolbox for frequency
decomposition of simulated wave data, including power spectral density
analysis, filtering, Hilbert transform, generalized phase, empirical mode
decomposition (EMD), and wavelet convolution.
"""
...
from WaveSpace.Decomposition import EMD, GenPhase, Hilbert, Morlet
from WaveSpace.PlottingHelpers import Plotting
from WaveSpace.Preprocessing import Filter
from WaveSpace.Utils import ImportHelpers, WaveData
...
# ----------------------------------
# We filter the data narrowly around our frequency of interest (10Hz) and then
...
waveData.add_data_bucket(WaveData.DataBucket(temp, "NBFiltered", "freq_trl_posx_posy_time", sampleRate=waveData.get_sample_rate(), chanNames=waveData.get_channel_names()))
# remove the individual filtered data
waveData.delete_data_bucket("10")
waveData.delete_data_bucket("17")
# get complex timeseries
Hilbert.apply_hilbert(waveData, dataBucketName="NBFiltered")
...
if not os.environ.get("BUILDING_DOCS"):
    ...
    waveData.save_to_file(output_dir / "complexData.pkl")

# %%
# Option 2: Generalized Phase
# ----------------------------
# Adapted from https://github.com/mullerlab/generalized-phase
lowerCutOff = 1
higherCutOff = 40
Filter.filter_broadband(waveData, "SimulatedData", lowerCutOff, higherCutOff, 5)
...
fig, axs = plt.subplots(2, 1, figsize=(10, 6), sharex=True)
fig.suptitle("Generalized Phase")
# real part and envelope
...
axs[0].legend()
axs[0].grid()
# note that we do not plot the real and the envelope of an analytic signal here (like we did above).
# This is because we did not narrowband filter before genPhase and the envelope of the real signal
# after genPhase would look very weird
# phase
...
# ---------------------------------------------
# If we cannot expect the signal to be well behaved for FFT based approaches,
# we can use EMD. Note that this is A LOT slower than Filter + Hilbert.
if not os.environ.get("BUILDING_DOCS"):
...
# Option 4: Wavelets
# -------------------
# 4.1 Time-Domain. Frequencies are the centre frequencies of the wavelets and
# would normally be logarithmically spaced within your frequency range of interest.
...
complexData = waveData.DataBuckets["complexData"].get_data()[0, 0, 18, 19, :]  # dimord is freq_trl_posx_posy_time
fig, axs = plt.subplots(2, 1, figsize=(10, 6), sharex=True)
fig.suptitle("Wavelet Convolution")
# real part and envelope
axs[0].plot(waveData.get_time(), np.real(complexData), label='Real part')
axs[0].plot(waveData.get_time(), np.abs(complexData), label='Envelope', linestyle='--')
axs[0].set_ylabel('Amplitude')
